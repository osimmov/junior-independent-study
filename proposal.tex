...
By focusing on reflection, transparency, and user agency, the system aims to support users in understanding their own work habits.
...
\caption{Report Panel Draft}
\label{fig:Report}       % Give a unique label
\end{center}
\end{figure}
...
Planned features:
\begin{itemize}
  \item User can create tasks assigned to a specific day of the week.
  \item Each task includes a title, optional description, creation timestamp, and completion status.
  \item User can mark tasks as completed or incomplete.
  \item User can edit or delete existing tasks.
  \item User can view a weekly timeline showing tasks organized by day.
  \item User can write a short daily reflection associated with each day.
  \item The system stores task and reflection data locally on the users machine.
  \item A local AI model analyzes task metadata and reflections to generate weekly productivity summaries.
  \item The AI provides observations about task load, completion trends, and pacing across the week.
  \item The AI suggests actionable productivity tips based on observed patterns.
  \item User can view historical AI feedback from previous weeks.
  \item (Stretch Goal) Task classification by type: academic, personal, work related.
  \item (Stretch Goal) Productivity metrics dashboard.
  \item (Stretch Goal) Task completion streak.
\end{itemize}
