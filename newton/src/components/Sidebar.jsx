...
function Sidebar({ activePanel, onSelect }) {
  return (
    <aside className="w-14 flex flex-col items-center py-4 border-r border-newton-border bg-newton-surface">
      {/* Horizon*/}
      <button
        type="button"
      ...
      >
        ...
        {activePanel === 'horizon' && (
          <div className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-6 bg-newton-text rounded-full" />
        )}
        <svg className={`w-6 h-6 ${activePanel === 'horizon' ? 'text-newton-text' : 'text-newton-muted'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      >
        ...
        <svg className={`w-6 h-6 ${activePanel === 'progress' ? 'text-newton-text' : 'text-newton-muted'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      >
        ...
        <svg className={`w-6 h-6 ${activePanel === 'insights' ? 'text-newton-text' : 'text-newton-muted'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
